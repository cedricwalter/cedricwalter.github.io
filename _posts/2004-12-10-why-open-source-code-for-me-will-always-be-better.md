...
permalink: /open-source/why-open-source-code-for-me-will-always-be-better/
tags:
    - distributions
    - linux
    - technology
...
But in real world, this conclusion is not realistic, since all webserver, and critical application are running under a UNIX system ([Google #Linux](http://www.researchchannel.org/program/displayevent.asp?rid=1680), and look [at the crazy uptime of the 50 first server](http://uptime.netcraft.com/up/today/top.avg.html) (average uptime of the first site is 1567 days. That’s over four years!!!) running in the world on [Netcraft](http://uptime.netcraft.com/up/graph): all are #Linux based)

For me it has to do with the quality of the code.

Linux is open for a long time (start in 1991, and based on Unix concept a lot more older) and so it has been hardened. M$’s code will be the opposite since the only model for security so far **has been** [**secret by obscurity**](http://en.wikipedia.org/wiki/Security_through_obscurity) (disastrous results as state by Kerckhoffs’ principle from the late 1880s) and **influencing lawmakers** to produce draconian laws to try to prevent anyone from considering trying to bypass the security. This has resulted in M$’s laxness in security that a few hyped media campaigns and a couple of code patches is not going to solve. Morever, since M$ can not beat Linux with code quality (at least not before 2-3 years), the concept of Patents has popup in Europe and outside America. These big company are not only trying to kill an utopy, it is like trying to close the mouth of all individual computer hobbyist all over the world. Imagine yourself in a world where an idea can be patented, what a disaster for innovation, since every technologic improvment is based more or less on what has been done before….
