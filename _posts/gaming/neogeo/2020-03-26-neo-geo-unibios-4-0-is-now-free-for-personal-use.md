...
guid: 'https://waltercedric.com/?p=11109'
...
- Not to included the [UNIVERSE BIOS](http://unibios.free.fr/) in any product that is sold.
- Not to sell the [UNIVERSE BIOS](http://unibios.free.fr/) on EPROM (or any other physical medium).
...
```
     ...
     (A)+(B)+(C)+(D)	</strong>Memory Card Manager<strong>

     (B)+(C)+(D)		</strong>Test Mode (MVS only)<strong>

     (B)+(C)+(D)		</strong>Hardware Test (AES only)<strong>

     (Use 2up controller for the following code)</strong>
```
...
```
    ...
    (START)+(COIN)		</strong>In Game Menu<strong>

    (START)+(A)+(B)+(C)	</strong>In Game Menu
```