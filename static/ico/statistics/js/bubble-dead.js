...
function main(country, divClass, displayLegend, autoscale, width = 1024, height = 768, legendNumber, year = '', category = '', fileType = 'dead.html') {
    var lines = 0;
    var totalRaised = 0;
    ...
    const file = "https://ico.tokens-economy.com/statistics/generated/" + country + ".csv";
...
function formatTitle(d) {
    ...
    var www = '<h4><a href="' + d.www + '" target="_blank">' + d.www + '</a></h4>';

    let raised = d.value !== 1.0001 ? '$' + format(d.value) + ' Million' : '???';

    return '<h1>' + d.name + '</h1>' +
        '<h2>' + category + '</h2>' +
	'<h2><i class="fas fa-skull fa-3x"></i></h2>' +
	'<h2><a href="' + d.link + '">' + d.reason + '</a></h2>' +
        '<h3>' + raised + ' </h3>' +
	'<h3>Lifetime: ' + d.lifetime + '</h3>' +
        '<h4>' + start + end + '<h4>' +
        www +
        ticker +
        '<p>' + desc + '</p>';
...
function addCountryLegend(divClass, country) {
    $(divClass + '-country').replaceWith(capitalizeFirstLetter(country.replace("%20", " ").replace("dead-", " ")));
}
...
function addLegend(displayLegend, divClass, lines, country, totalRaised, displayLegendNumber, fileType) {
	...
	if (!legends[i]) {
	 console.log("in country " + country + " i " + i + " legends " + legends);
        ...
        text = text + "<a href='" + fileType + "?"+ selector +"=" + legend + "' class='tooltip' title='Click to display all ICO in category \"" + legend + "\" only.'><b style='color: " + color + "'><i class=\"fas fa-square\"></i> " + itsLegendNumber + legend + "</b></a>&nbsp; ";
    }
    text = text + "";
