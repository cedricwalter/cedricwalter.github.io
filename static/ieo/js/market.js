...
function redraw() {
    ...
    country = getParamWithDefault('country', 'overall');
    ...
    const fileType = getParamWithDefault('fileType', 'market.html');
...
function main(country, divClass, displayLegend, autoscale, width = 1024, height = 768, legendNumber, year = '', category = '', fileType = 'market.html') {
    var lines = 0;
    var totalRaised = 0;
    ...
    const file = "https://ieo.tokens-economy.com/generated/overall.csv";
...
function formatTitle(d) {
    ...
    var www = "";
    if (d.www) {
        www = '<h4><a href="' + d.www + '?utm_source=ico.tokens-economy.com' +  '" target="_blank">' + d.www.replace('?utm_source=ico.tokens-economy.com', '') + '</a></h4>';
    }
    let raised = d.value !== 1.0001 ? '$' + format(d.value) + ' Million' : '???';

    if (d.name) {
        return '<h1><img src="https://ieo.tokens-economy.com/generated/images/icos/icons/' + d.name.replace(' ', '-').replace('%20', '-') + '.jpg"></h1>' +
            '<h1>' + d.name + '</h1>' +
            '<h2>' + category + '</h2>' +
            '<h3>' + raised + ' </h3>' +
            '<h4>' + start + end + '<h4>' +
