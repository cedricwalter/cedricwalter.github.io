...
function main(country, divClass, displayLegend, autoscale, width = 1024, height = 768, legendNumber, year = '', category = '', fileType = 'market.html') {
    ...
    const file = "https://sto.tokens-economy.com/generated/overall.csv";
...
function formatTitle(d) {
    ...
    var www = '<h4><a href="' + d.www + '" target="_blank">' + d.www.replace('?utm_source=ico.tokens-economy.com','') + '</a></h4>';

    let raised = d.value !== 1.0001 ? '$' + format(d.value) + ' Million' : '???';

    return '<h1><img src="https://sto.tokens-economy.com/generated/images/icos/icons/' + d.name.replace(' ','-').replace('%20','-') + '.jpg"></h1>' +
    '<h1>' + d.name + '</h1>' +
        '<h2>' + category + '</h2>' +
        '<h3>' + raised + ' </h3>' +
        '<h4>' + start + end + '<h4>' +
